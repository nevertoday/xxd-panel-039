...
| 模式 | 画幅逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在上，039 设计在下，约 50/50 |
| `left-right` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在左，039 设计在右，约 50/50 |
| `design-only` | 用户确认的整张成品画幅 | 039 设计铺满画布，不显示原照片 |
...
本项目的文案规则： 从没有直接说出的情绪、动作、关系或寓意提炼一个极短词或短句，用目标语言的细线刺绣、纤细题签或极简小字呈现，顺着游丝、进入负空间或贴近绣面边缘。
...
```text
/xxd-panel-039 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text auto --locale ja-JP
```
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/039-source.md) · [英文生产提示词](references/xxd-panel-039-prompt.en.md) · [中文生产提示词](references/xxd-panel-039-prompt.zh-CN.md)
